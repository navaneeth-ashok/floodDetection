...
abc = np.zeros(overlap.shape, np.uint8)
...
cv2.drawContours(mask,[cnt],0,(255,0,0),3)

cv2.drawContours(overlap, [cnt], 0, (0,255,0), 3)
#mean_val = cv2.mean(overlap, mask = abc)
#print "average = " + str(mean_val)
cv2.imshow('trial',overlap)
cv2.imshow('trial2',mask)

res = cv2.bitwise_and(im,im, mask= mask)
cv2.imshow('image-and',im)
